...
namespace jwt {
	...
	namespace error {
		...
		inline void throw_if_error(std::error_code ec) {
			UE_LOG(LogTemp, Warning, TEXT("throw_if_error reached"));
			return;
		}
	} // namespace error
} // namespace jwt
	...
	namespace helper {
		...
		private:
			...
			evp_pkey_handle(const evp_pkey_handle& other) : m_key{other.m_key} {
				if (m_key != nullptr && EVP_PKEY_up_ref(m_key) != 1) UE_LOG(LogTemp, Warning, TEXT("EVP_PKEY_up_ref failed"));
			}
// C++11 requires the body of a constexpr constructor to be empty
		...
		private:
			...
			static void increment_ref_count(EVP_PKEY* key) {
				if (key != nullptr && EVP_PKEY_up_ref(key) != 1) UE_LOG(LogTemp, Warning, TEXT("EVP_PKEY_up_ref failed"));
			}
	...
	namespace algorithm {
		...
		struct none {
			 ...
			 */
			void jwt_verify(const std::string& /*unused*/, const std::string& signature, std::error_code& ec) const {
				ec.clear();
		...
		struct hmacsha {
			 ...
			 */
			void jwt_verify(const std::string& data, const std::string& signature, std::error_code& ec) const {
				ec.clear();
		...
		struct rsa {
			...
			rsa(const std::string& public_key, const std::string& private_key, const std::string& public_key_password,
				...
				if (!private_key.empty()) {
					...
					UE_LOG(LogTemp, Warning, TEXT("no rsa key provided"));
			}
			/**
			 * Sign jwt data
			 * \param data The data to sign
		...
		struct ecdsa {
			...
			ecdsa(const std::string& public_key, const std::string& private_key, const std::string& public_key_password,
				...
				if (!private_key.empty()) {
					...
					UE_LOG(LogTemp, Warning, TEXT("no ecdsa key provided"));
				}
				if (!pkey) UE_LOG(LogTemp, Warning, TEXT("ecdsa key invalid"));

				size_t keysize = EVP_PKEY_bits(pkey.get());
				if (keysize != signature_length * 4 && (signature_length != 132 || keysize != 521))
					UE_LOG(LogTemp, Warning, TEXT("ecdsa key wrong size"));
			}
		...
		private:
			static void check_public_key(EVP_PKEY* pkey) {
				...
				if (!ctx) { UE_LOG(LogTemp, Warning, TEXT("unable to create ecdsa key context")); }
				if (EVP_PKEY_public_check(ctx.get()) != 1) {
					UE_LOG(LogTemp, Warning, TEXT("ecdsa key invalid"));
				}
#else
				std::unique_ptr<EC_KEY, decltype(&EC_KEY_free)> eckey(EVP_PKEY_get1_EC_KEY(pkey), EC_KEY_free);
				if (!eckey) { UE_LOG(LogTemp, Warning, TEXT("invalid ecdsa key")); }
				if (EC_KEY_check_key(eckey.get()) == 0) UE_LOG(LogTemp, Warning, TEXT("ecdsa key invalid"));
#endif
			}
			...
			static void check_private_key(EVP_PKEY* pkey) {
				...
				std::unique_ptr<EVP_PKEY_CTX, decltype(&EVP_PKEY_CTX_free)> ctx(
					EVP_PKEY_CTX_new_from_pkey(nullptr, pkey, nullptr), EVP_PKEY_CTX_free);
				if (!ctx) { UE_LOG(LogTemp, Warning, TEXT("unable to create ecdsa context")); }
				if (EVP_PKEY_private_check(ctx.get()) != 1) {
					UE_LOG(LogTemp, Warning, TEXT("ecdsa key invalid"));
				}
#else
				std::unique_ptr<EC_KEY, decltype(&EC_KEY_free)> eckey(EVP_PKEY_get1_EC_KEY(pkey), EC_KEY_free);
				if (!eckey) { UE_LOG(LogTemp, Warning, TEXT("ecdsa key invalid")); }
				if (EC_KEY_check_key(eckey.get()) == 0) UE_LOG(LogTemp, Warning, TEXT("ecdsa key invalid"));
#endif
			}
			...
			std::string der_to_p1363_signature(const std::string& der_signature, std::error_code& ec) const {
				...
				if (rr.size() > signature_length / 2 || rs.size() > signature_length / 2)
					UE_LOG(LogTemp, Warning, TEXT("bignum size exceeded expected length"));
		...
		struct eddsa {
			...
			eddsa(const std::string& public_key, const std::string& private_key, const std::string& public_key_password,
				...
				if (!private_key.empty()) {
					...
					UE_LOG(LogTemp, Warning, TEXT("load ecdsa key bio read"));
			}
			/**
			 * Sign jwt data
			 * \param data The data to sign
...
// sets the salt length to the hash length. Unlike OpenSSL which exposes this functionality.
	...
	namespace details {
		...
		struct map_of_claims {
			...
			static typename json_traits::object_type parse_claims(const typename json_traits::string_type& str) {
				typename json_traits::value_type val;
				if (!json_traits::parse(val, str)) UE_LOG(LogTemp, Warning, TEXT("invalid json"));
			...
			basic_claim_t get_claim(const typename json_traits::string_type& name) const {
				if (!has_claim(name)) UE_LOG(LogTemp, Warning, TEXT("claim not present"));
	...
	public:
		...
		decoded_jwt(const typename json_traits::string_type& token, Decode decode) : token(token) {
			...
			if (hdr_end == json_traits::string_type::npos) UE_LOG(LogTemp, Warning, TEXT("invalid token supplied"));
			auto payload_end = token.find('.', hdr_end + 1);
			if (payload_end == json_traits::string_type::npos) UE_LOG(LogTemp, Warning, TEXT("invalid token supplied"));
		...
		struct equals_claim {
			...
			void operator()(const verify_context<json_traits>& ctx, std::error_code& ec) const {
				...
				const bool matches = [&]() {
					...
					default: UE_LOG(LogTemp, Warning, TEXT("internal error, should be unreachable"));
					}
				}();
				if (!matches) {
	...
	private:
		struct algo_base {
			...
			virtual void jwt_verify(const std::string& data, const std::string& sig, std::error_code& ec) = 0;
		};
		template<typename T>
		struct algo : public algo_base {
			T alg;
			explicit algo(T a) : alg(a) {}
			void jwt_verify(const std::string& data, const std::string& sig, std::error_code& ec) override {
				alg.jwt_verify(data, sig, ec);
			}
		};
		/// Required claims
	...
	public:
		 ...
		 */
		void jwt_verify(const decoded_jwt<json_traits>& jwt) const {
			std::error_code ec;
			jwt_verify(jwt, ec);
			error::throw_if_error(ec);
		}
		 ...
		 * \param ec error_code filled with details on error
		 */
		void jwt_verify(const decoded_jwt<json_traits>& jwt, std::error_code& ec) const {
			ec.clear();
			...
			algs.at(algo)->jwt_verify(data, sig, ec);
			if (ec) return;
	...
	public:
		...
		typename json_traits::string_type get_x5c_key_value() const {
			...
			if (x5c_array.size() == 0) UE_LOG(LogTemp, Warning, TEXT("claim not present"));
	...
	public:
		...
		JWT_CLAIM_EXPLICIT jwks(const typename json_traits::string_type& str) {
			...
			if (!json_traits::parse(parsed_val, str)) UE_LOG(LogTemp, Warning, TEXT("invalid json"));

			const details::map_of_claims<json_traits> jwks_json = json_traits::as_object(parsed_val);
			if (!jwks_json.has_claim("keys")) UE_LOG(LogTemp, Warning, TEXT("invalid json"));
		...
		jwk_t get_jwk(const typename json_traits::string_type& key_id) const {
			...
			if (maybe == end()) UE_LOG(LogTemp, Warning, TEXT("claim not present"));
			return *maybe;
		}
	 ...
	 */
	template<typename Clock, typename json_traits>
	verifier<Clock, json_traits> jwt_verify(Clock c) {
	 ...
	 */
	template<typename json_traits>
	verifier<default_clock, json_traits> jwt_verify(default_clock c = {}) {
